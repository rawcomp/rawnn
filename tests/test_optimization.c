...
static void assert_matrix_close(const RMatrix *m, const float *expected,
				size_t n, const char *label)
{
	for (size_t i = 0; i < n; i++) {
		char msg[128];
		snprintf(msg, sizeof(msg), "%s[%zu]", label, i);
		assert_float_close(m->data[i], expected[i], DEFAULT_ABS_TOL,
				   DEFAULT_REL_TOL, msg);
	}
}
...
static void test_sgd_negative_grad(void)
{
	...
	assert_float_close(theta->data[0], 1.1f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgd_negative_grad");
...
static void test_sgd_zero_grad(void)
{
	...
	assert_float_close(theta->data[0], 3.14f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgd_zero_grad[0]");
	assert_float_close(theta->data[1], -2.71f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgd_zero_grad[1]");
...
static void test_sgd_zero_eta(void)
{
	...
	assert_float_close(theta->data[0], 5.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgd_zero_eta[0]");
	assert_float_close(theta->data[1], -3.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgd_zero_eta[1]");
...
static void test_sgd_nan_grad_propagates(void)
{
	...
	assert(isnan(theta->data[0]) &&
	       "sgd: NaN in gradient should propagate to theta");
...
static void test_sgdm_basic_zero_velocity(void)
{
	...
	assert_float_close(velocity->data[0], v0, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_velocity[0]");
	assert_float_close(velocity->data[1], v1, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_velocity[1]");
	assert_float_close(theta->data[0], 1.0f - eta * v0, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_theta[0]");
	assert_float_close(theta->data[1], 2.0f - eta * v1, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_theta[1]");
...
static void test_sgdm_beta_zero_equals_sgd(void)
{
	...
	for (size_t i = 0; i < 2; i++) {
		...
		assert_float_close(theta_sgdm->data[i], theta_sgd->data[i],
...
static void test_sgdm_beta_one_ignores_grad(void)
{
	...
	assert_float_close(theta->data[0], 2.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_beta1_no_change");
...
static void test_sgdm_velocity_accumulates(void)
{
	...
	assert_float_close(velocity->data[0], v2, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_velocity_step2");
...
static void test_sgdm_zero_eta(void)
{
	...
	assert_float_close(theta->data[0], 5.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "sgdm_zero_eta_no_change");
...
static void test_rmsprop_basic(void)
{
	...
	assert_float_close(cache->data[0], c, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "rmsprop_cache");
	assert_float_close(theta->data[0], 2.0f - step, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "rmsprop_theta");
...
static void test_rmsprop_rho_zero(void)
{
	...
	assert_float_close(cache->data[0], c, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "rmsprop_rho0_cache");
	assert_float_close(theta->data[0], 1.0f - step, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "rmsprop_rho0_theta");
...
static void test_rmsprop_zero_grad(void)
{
	...
	assert_float_close(cache->data[0], 0.45f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "rmsprop_zero_grad_cache");
	assert_float_close(theta->data[0], 3.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "rmsprop_zero_grad_theta");
...
static void test_rmsprop_cache_accumulates(void)
{
	...
	assert_float_close(cache->data[0], c2, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "rmsprop_cache_step2");
...
static void test_adam_basic_t1(void)
{
	...
	assert_float_close(m->data[0], m_new, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "adam_m_t1");
	assert_float_close(v->data[0], v_new, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "adam_v_t1");
	assert_float_close(theta->data[0], 1.0f - step, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "adam_theta_t1");
...
static void test_adam_zero_grad(void)
{
	...
	assert_float_close(theta->data[0], 3.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "adam_zero_grad_theta");
	assert_float_close(m->data[0], 0.0f, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "adam_zero_grad_m");
...
static void test_adam_zero_eta(void)
{
	...
	assert_float_close(theta->data[0], 7.0f, DEFAULT_ABS_TOL,
			   DEFAULT_REL_TOL, "adam_zero_eta_theta");
...
static void test_adam_two_steps(void)
{
	...
	assert_float_close(m->data[0], m2, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "adam_m_step2");
	assert_float_close(v->data[0], v2, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "adam_v_step2");
...
static void test_adam_large_t(void)
{
	...
	r_optimization_adam(theta, grad, m, v, 0.001f, 0.9f, 0.999f, 1e-8f,
			    10000);
...
static void test_adam_nan_grad_propagates(void)
{
	...
	assert(isnan(theta->data[0]) &&
	       "adam: NaN in gradient should propagate to theta");
...
static void test_all_optimizers_empty_matrix(void)
{
	...
	r_optimization_adam(theta, grad, aux1, aux2, 0.001f, 0.9f, 0.999f,
			    1e-8f, 1);
