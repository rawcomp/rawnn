...
static float numeric_bin_focal_grad(const RMatrix *pred, const RMatrix *real,
				    float gamma, float alpha, size_t idx)
{
	...
	const float loss_minus =
	    r_bin_focal_loss(pred_minus, real, gamma, alpha);
...
static float numeric_cat_focal_grad(const RMatrix *pred, const RMatrix *real,
				    float gamma, size_t idx)
{
	const float delta = 1e-4f;
...
static void test_cross_entropy_grad_basic(void)
{
	...
	for (size_t i = 0; i < pred->rows; i++) {
		for (size_t j = 0; j < pred->cols; j++) {
			const size_t idx = R_MATRIX_IDX(i, j, pred->cols);
			const float p = pred->data[idx];
			const float y = real->data[idx];
			const float expected = -y / (2.0f * (p + EPSILON));

			char msg[96];
			snprintf(msg, sizeof(msg), "cross_entropy_grad idx=%zu",
				 idx);
			assert_float_close(grad->data[idx], expected,
...
static void test_cross_entropy_grad_zero_prediction(void)
{
	...
	assert_float_close(grad->data[0], expected, 1e-2f, 1e-5f,
			   "cross_entropy_grad handles p=0 via EPSILON");
	...
	for (size_t i = 0; i < pred->rows; i++) {
		for (size_t j = 0; j < pred->cols; j++) {
			...
			snprintf(msg, sizeof(msg),
				 "cat_vs_cross_entropy_grad idx=%zu", idx);
			assert_float_close(grad_cat->data[idx],
					   grad_ce->data[idx], DEFAULT_ABS_TOL,
					   DEFAULT_REL_TOL, msg);
		}
	}
...
static void test_bin_cross_entropy_grad_basic(void)
{
	...
	for (size_t i = 0; i < pred->rows; i++) {
		for (size_t j = 0; j < pred->cols; j++) {
			...
			char msg[96];
			snprintf(msg, sizeof(msg),
				 "bin_cross_entropy_grad idx=%zu", idx);
			assert_float_close(grad->data[idx], expected,
					   DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
					   msg);
		}
...
static void test_mse_loss_grad_basic(void)
{
	...
	for (size_t i = 0; i < 4; i++) {
		...
		assert_float_close(grad->data[i], expected_vals[i],
...
static void test_bin_focal_loss_grad_numeric(void)
{
	...
	for (size_t i = 0; i < 2; i++) {
		const float expected =
		    numeric_bin_focal_grad(pred, real, gamma, alpha, i);
		char msg[96];
...
static void test_cat_focal_loss_grad_numeric(void)
{
	...
	for (size_t i = 0; i < 3; i++) {
		const float expected =
		    numeric_cat_focal_grad(pred, real, gamma, i);
		char msg[96];
...
static void test_zero_sized_matrix_no_crash(void)
{
	...
	assert(pred->rows == 0 && pred->cols == 0 &&
	       "zero-sized matrix constructed");
