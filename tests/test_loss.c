...
static void test_cross_entropy_basic(void)
{
	...
	const float expected =
	    -(logf(0.7f + EPSILON) + logf(0.8f + EPSILON)) / 2.0f;
	const float actual = r_cross_entropy(pred, real);

	assert_float_close(actual, expected, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "cross_entropy basic");
...
static void test_cross_entropy_matches_cat_cross_entropy(void)
{
	...
	assert_float_close(cat, ce, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "cat_cross_entropy matches cross_entropy");
...
static void test_bin_cross_entropy_basic(void)
{
	...
	const float expected =
	    -(logf(0.9f + EPSILON) + logf(1.0f - 0.1f + EPSILON) +
	      logf(1.0f - 0.2f + EPSILON) + logf(0.8f + EPSILON)) /
	    4.0f;

	const float actual = r_bin_cross_entropy(pred, real);

	assert_float_close(actual, expected, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "bin_cross_entropy basic");
...
static void test_cat_focal_gamma_zero_matches_cat_ce(void)
{
	...
	assert_float_close(focal, ce, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "cat_focal gamma=0 equals cat_ce");
...
static void test_cat_focal_all_zero_targets_is_zero(void)
{
	...
	assert_float_close(loss, 0.0f, DEFAULT_ABS_TOL, DEFAULT_REL_TOL,
			   "cat_focal all zero targets");
...
static void test_zero_sized_matrices_return_nan(void)
{
	...
	assert_isnanf(r_cat_cross_entropy(pred, real),
		      "cat_cross_entropy on empty");
	assert_isnanf(r_bin_cross_entropy(pred, real),
		      "bin_cross_entropy on empty");
	assert_isnanf(r_mse_loss(pred, real), "mse_loss on empty");
	assert_isnanf(r_mae_loss(pred, real), "mae_loss on empty");
	assert_isnanf(r_bin_focal_loss(pred, real, 2.0f, 0.25f),
		      "bin_focal_loss on empty");
	assert_isnanf(r_cat_focal_loss(pred, real, 2.0f),
		      "cat_focal_loss on empty");
